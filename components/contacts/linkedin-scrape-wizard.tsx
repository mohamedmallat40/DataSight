...
export default function LinkedInScrapeWizard({ onClose, onSuccess }: LinkedInScrapeWizardProps) {
  const [currentStep, setCurrentStep] = useState(1);
  const [linkedinUrl, setLinkedinUrl] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProfile, setScrapedProfile] = useState<LinkedInProfile | null>(null);
  const [extractedContact, setExtractedContact] = useState<Partial<Users> | null>(null);
  ...
  const handleUrlSubmit = async () => {
    ...
    const mockProfile: LinkedInProfile = {
      ...
      about: "Passionate software engineer with expertise in full-stack development, cloud architecture, and team leadership."
    };
      ...
      >
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <Icon icon="solar:link-linear" className="text-primary" width={20} />
                <h3 className="text-lg font-semibold">Enter LinkedIn Profile URL</h3>
              </div>
            </CardHeader>
            <CardBody className="space-y-6">
              <div>
                <Input
                  ...
                  isInvalid={linkedinUrl.length > 0 && !isValidLinkedInUrl(linkedinUrl)}
                  errorMessage={linkedinUrl.length > 0 && !isValidLinkedInUrl(linkedinUrl) ? "Please enter a valid LinkedIn profile URL" : ""}
                />
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-950/20 p-4 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-small text-blue-800 dark:text-blue-200">
                    <p className="font-medium mb-2">How to find a LinkedIn profile URL:</p>
                    <ol className="list-decimal list-inside space-y-1 text-tiny">
                      ...
                      <li>Copy the entire URL (e.g., https://www.linkedin.com/in/john-smith)</li>
              ...
              <div className="flex justify-between">
                ...
                <Button
                  ...
                  isDisabled={!linkedinUrl.trim() || !isValidLinkedInUrl(linkedinUrl)}
                  isLoading={isLoading}
                  startContent={!isLoading ? <Icon icon="solar:eye-linear" /> : undefined}
                >
        ...
        {currentStep === 2 && scrapedProfile && (
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <Icon icon="solar:eye-linear" className="text-primary" width={20} />
                <h3 className="text-lg font-semibold">Profile Analysis Results</h3>
              </div>
            </CardHeader>
            <CardBody className="space-y-6">
              <div className="flex items-start gap-4 p-4 bg-success-50 dark:bg-success-900/20 rounded-lg">
                <Avatar src={scrapedProfile.avatar} size="lg" />
                <div className="flex-1">
                  <h4 className="text-lg font-semibold text-foreground">{scrapedProfile.name}</h4>
                  <p className="text-default-600 mb-2">{scrapedProfile.headline}</p>
                  <div className="flex flex-wrap gap-2 mb-3">
                    <Chip startContent={<Icon icon="solar:map-point-linear" width={14} />} size="sm" variant="flat">
                      {scrapedProfile.location}
                    </Chip>
                    <Chip startContent={<Icon icon="solar:users-group-rounded-linear" width={14} />} size="sm" variant="flat">
                      {scrapedProfile.connections}
                    </Chip>
                    <Chip startContent={<Icon icon="solar:case-linear" width={14} />} size="sm" variant="flat">
                      {scrapedProfile.experience}
                    </Chip>
                  </div>
                  <p className="text-small text-default-500">{scrapedProfile.about}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{scrapedProfile.headline.split(" at ")[0]}</p>
                  </div>
                  <div>
        ...
        {currentStep === 3 && extractedContact && (
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <Icon icon="solar:check-circle-linear" className="text-primary" width={20} />
            ...
            <CardBody className="space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                <Input
                  ...
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, full_name: value} : null)}
                />
                <Input
                  label="Job Title"
                  value={extractedContact.job_title || ""}
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, job_title: value} : null)}
                />
                <Input
                  label="Company"
                  value={extractedContact.company_name || ""}
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, company_name: value} : null)}
                />
                <Input
                  label="LinkedIn URL"
                  value={extractedContact.linkedin || ""}
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, linkedin: value} : null)}
                />
                <Input
                  label="City"
                  value={extractedContact.city || ""}
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, city: value} : null)}
                />
                <Input
                  label="Country"
                  value={extractedContact.country || ""}
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, country: value} : null)}
                />
              </div>
              ...
              <div>
                <Input
                  ...
                  onValueChange={(value) => setExtractedContact(prev => prev ? {...prev, notes: value} : null)}
